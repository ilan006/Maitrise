            ...
            for question in paragraph['qas']:
                ...
                for answer in question['answers']:
                    ...
                    if(  word_tokenize(answer['text'])[0] != word_tokenize(paragraph['context'])[position_Word]):
                        # print(word_tokenize(answer['text'])[0])
                        # print(word_tokenize(paragraph['context'])[position_Word+1])
                        # print("diff")
                        print(question['question'])
                        print('id', question['id'])
                        print("mot reponse :", answer['text'])
                        print("position" , position_Word)
                        print("position_char", pos)
                        print(paragraph['context'])
                        print("mot texte", word_tokenize(paragraph['context']))
                        print("mot sorti", word_tokenize(paragraph['context'])[position_Word])
                        time.sleep(3)
                    if (word_tokenize(answer['text'])[0] == word_tokenize(paragraph['context'])[position_Word]):
                        pareil += 1
