'''
Detection des spans réponse à l'aide des entité nommée avec sauvegardes des erreurs dans un fichier text
'''
...
with open(path_data + 'data_question_'+type_question_str+'_'+selected_data+'.json', 'r') as input:
    d = json.load(input)
    input.close()
with open(path_dest + 'data_question_' + type_question_str + '_' + selected_data + '.txt', 'w') as outfile:
    ...
    for id in d:
        ...
        for sentence in list_sentence_answer:
            ...
            if len(list_ent_data) == 0:
                ...
                outfile.write("question " + str(id) + " = " + question + "\n")
                outfile.write("phrase " + str(id) + " = " + sentence + "\n")
                outfile.write("réponses " + str(id) + " = " + str(list_answers) + "\n")
                outfile.write("list_reponses_proposées " + str(id) + " = " + str(list_ent_data) + "\n")
                ...
                for answer in list_answers:
                    if normalize_answer(answer) in normalize_answer(concatenation_ent):
                        ...
                        break
                if not normalize_answer(answer) in normalize_answer(concatenation_ent):
                    outfile.write("question " + str(id) + " = " + question + "\n")
                    outfile.write("phrase " + str(id) + " = " + sentence + "\n")
                    outfile.write("réponses " + str(id) + " = " + str(list_answers) + "\n")
                    outfile.write("list_reponses_proposées " + str(id) + " = " + str(list_ent_data) + "\n")
